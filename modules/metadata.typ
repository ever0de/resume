...
#let metadata = (
  ...
  bio: (
    ko: [시스템 내부가 실제로 어떻게 작동하는지 파고드는 것을 즐기는 엔지니어입니다. \
      GlueSQL 멘티·멘토 (3년 연속 최우수상), RustPython, CPython, pebble 등 150+ PRs 기여],
    en: [An engineer who enjoys digging into how systems actually work under the hood. \
      GlueSQL mentee·mentor (3 consecutive Grand Prizes), RustPython, CPython, pebble, and more — 150+ PRs.],
  ),
  skills: (
  ...
  work: (
    (
      ...
      tags: ("Rust", "Go", "gRPC", "pebble", "Kubernetes"),
      body-ko: [
        분산 KV 스토리지 시스템 전체를 처음부터 설계·구현 및 운영.

        - 수평 확장이 불가능한 단일 노드 구조를 수평 확장 가능하게 전환, 동일 하드웨어 기존 구현 대비 17.5배 처리량 개선 (피크 트래픽 기준).
          ...
          - lock-free arena skiplist MVCC: 메모리 상한 도달 시 ring-buffer 방식의 슬롯 관리와 atomic reference counting으로 진행 중인 읽기의 snapshot consistency를 보장하면서 arena를 안전하게 교체.
          - sparse key space에서 range query 캐시 정합성 확보: 이미 조회한 구간 정보를 별도 인덱스로 추적해 불필요한 DB 재조회를 방지.
          - 네트워크 레이어를 epoll 기반 이벤트 루프 위에서 TCP를 직접 처리하는 구조(Netty reactor 패턴)로 구현. I/O 멀티플렉싱과 쿼리 처리를 분리해 이벤트 루프 블로킹 없이 쿼리별 goroutine의 CPU 병렬성 확보, 동일 크기 버퍼 재사용으로 메모리 할당 pressure 감소. QUIC는 저지연 내부 네트워크 환경에서 UDP 기반 오버헤드가 이득을 상쇄함을 wrk로 검증 후 기각.
        ...
        - Pebble iterator latency가 기준치 대비 50배 수준으로 급등. 특정 쿼리에서 iterator stats의 point count가 비정상적으로 폭증함을 확인한 뒤 compaction picker 소스를 직접 추적해 move compaction 조건에서 tombstone이 미처리 상태로 L6에 누적되는 구조와, snapshot 보존 설정이 elision-only compaction을 차단하는 상호작용을 식별. L6 tombstone 전용 compaction을 적용해 정상 latency로 복구.
        - 다수의 프로덕션 장애 postmortem. 증상이 아닌 원인 수준까지 추적: 메모리 풀 재사용과 비동기 타이밍의 상호작용으로 인한 데이터 오염, 스토리지 엔진 상태 손상으로 인한 서비스 재시작 루프 등.
        - Rust 기반 Go 소스코드 자동 패치 CLI 도구 설계 및 개발. tree-sitter AST 패턴 매칭으로 외부 의존성 커스터마이징을 자동화.
        - Kubernetes operator 기반 배포 자동화 툴 설계 및 구현. 외부 시스템 상태 폴링으로 목표 조건 도달 시 이미지를 자동 교체하고, Slack Webhook 알림으로 자동 배포 파이프라인 구축.
      ],
      body-en: [
        Designed and built a distributed KV storage system from the ground up.

        - Converted a non-horizontally-scalable single-node architecture into a horizontally scalable one, achieving 17.5× throughput improvement over prior implementation on identical hardware (under peak traffic).
          ...
          - Lock-free arena skiplist MVCC: on memory limit, ring-buffer slot management with atomic reference counting ensures snapshot consistency for in-flight reads while safely rotating arenas.
          ...
          - Implemented the network layer as an epoll-based event loop handling raw TCP (Netty reactor pattern): separating I/O multiplexing from query processing avoids event-loop blocking and gains per-goroutine CPU parallelism, while fixed-size buffer reuse reduces allocation pressure. QUIC was evaluated but rejected after wrk confirmed UDP overhead negated any gains in a low-latency internal network.
        - The existing time travel query implementation stored each version independently, forcing iterators to scan tombstones across every version boundary; replaced with CoW snapshot checkpoints as periodic baselines and delta merge-on-read for intermediate versions, minimizing both scan overhead and storage footprint.
        - Pebble iterator latency spiked ~50× over baseline. A specific query showed point count in iterator stats exploding well beyond expected range, revealing excessive tombstone scanning; traced the compaction picker source to find tombstones accumulating in L6 unprocessed under move compaction conditions and snapshot retention blocking elision-only compaction. Applied L6-tombstone-targeted compaction to restore normal latency.
        - Root-cause analysis across multiple production incidents: memory pool reuse combined with async timing causing data corruption; storage engine state corruption causing service restart loops; etc.
        - Designed and built govld, a Rust CLI for automated Go source patching. tree-sitter AST pattern matching automated external dependency customization.
        - Kubernetes operator-based deployment automation tool. Polls external system state and auto-replaces images on condition match; Slack Webhook notifications for a fully automated deployment pipeline.
      ],
    ),
    (
      title-ko: "소프트웨어 엔지니어",
      ...
      body-ko: [
        ...
        - JS로 구현된 이미지 조합기의 성능 문제를 Rust로 재구현, 병렬 스레드풀로 생성 시간 60배 단축.
      ...
      body-en: [
        ...
        - Rewrote a JS image compositor in Rust; thread-pool-based parallel compositing cut generation time by 60×.
  ...
  awards: (
    (
      title-ko: "[멘토] 2023 오픈소스 컨트리뷰션 아카데미 GlueSQL",
      title-en: "[Mentor] 2023 OSS Contribution Academy GlueSQL",
      issuer-ko: "정보통신산업진흥원장상 (최우수상)",
      issuer-en: "NIPA Commissioner's Award (Grand Prize)",
      date: "2023.12",
    ),
    (
      title-ko: "[멘토] 2022 오픈소스 컨트리뷰션 아카데미 GlueSQL",
      title-en: "[Mentor] 2022 OSS Contribution Academy GlueSQL",
      issuer-ko: "정보통신산업진흥원장상 (최우수상)",
      issuer-en: "NIPA Commissioner's Award (Grand Prize)",
      date: "2022.11",
    ),
    (
      title-ko: "[멘티] 2021 오픈소스 컨트리뷰션 아카데미 GlueSQL",
      title-en: "[Mentee] 2021 OSS Contribution Academy GlueSQL",
      issuer-ko: "정보통신산업진흥원장상 (최우수상)",
      issuer-en: "NIPA Commissioner's Award (Grand Prize)",
      date: "2021.11",
    ),
  ...
  oss: (
    ...
    (
      ...
      period-ko: "2021.08 – 현재",
      period-en: "Aug 2021 – Present",
      tags: ("Rust",),
      stats: "★3k · 89 PRs",
      about-ko: "Rust 기반 이식형 SQL 쿼리 엔진.",
      about-en: "Portable SQL query engine in Rust.",
      contrib-ko: "오픈소스 컨트리뷰션 아카데미 2021 멘티 → 2022–2023 멘토, 콜라보레이터.",
      contrib-en: "OSS Contribution Academy: 2021 mentee → 2022–2023 mentor, collaborator.",
    ),
    (
      project: "pyre",
      repo: "youknowone/pyre",
      url: "https://github.com/youknowone/pyre/pull/1",
      period-ko: "2026.04",
      period-en: "Apr 2026",
      tags: ("Rust",),
      stats: "★69 · 1 PR",
      about-ko: "PyPy(meta-tracing JIT) 아키텍처를 Rust로 재구현한 no-GIL Python 인터프리터.",
      about-en: "no-GIL Python interpreter rebuilding PyPy's meta-tracing JIT architecture in Rust.",
      contrib-ko: [#link("https://morepypy.blogspot.com/2011/10/more-compact-lists-with-list-strategies.html")[list strategy] 미적용으로 object fallback만 타던 Integer·Float 리스트의 insert/pop/remove/reverse를 per-strategy로 구현.],
      contrib-en: [Implemented per-strategy insert/pop/remove/reverse for Integer·Float lists that were falling back to object strategy; ref: #link("https://morepypy.blogspot.com/2011/10/more-compact-lists-with-list-strategies.html")[list strategies].],
    ),
    (
    ...
    (
      ...
      period-ko: "2024 – 2025.12",
      period-en: "2024 – Dec 2025",
      tags: ("Rust",),
      stats: "author · ★17",
    ...
    (
      ...
      repo: "apache/datafusion-sqlparser-rs",
      url: "https://github.com/apache/datafusion-sqlparser-rs/pull/334",
      period-ko: "2021.08",
      period-en: "Aug 2021",
      tags: ("Rust",),
      stats: "★3.3k · 2 PRs",
