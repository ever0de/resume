...
// ── 수상 목록 ─────────────────────────────────────────────────────────────────
#let awards-list(awards, lang: "ko") = {
  table(
    ...
    ..awards
      .map(a => {
        let title = if lang == "ko" { a.title-ko } else { a.title-en }
        let issuer = if lang == "ko" { a.issuer-ko } else { a.issuer-en }
        (
          text(weight: 600, size: 9.5pt)[#title],
          align(right)[#text(fill: subtle, size: 8.5pt)[#issuer]],
          align(right)[#text(fill: subtle, size: 8.5pt)[#a.date]],
        )
      })
      .flatten(),
  )
