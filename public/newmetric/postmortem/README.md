...
Newmetric에서 블록체인 인프라(Cosmos SDK 기반) 운영 중 발생한 장애 대응 및 문제 해결 사례 모음.
...
2023-10 ~ 2025-02 동안 블록체인 인프라 운영 중 발생한 약 18건의 장애 이력.
...
Cosmos SDK v0.46 이전 버전까지 지원해야 하는 체인들은 tx execution 내부에서 별도의 goroutine을 태우지 않는 **싱글 스레드 순차 실행** 방식으로 구현되어 있었다. 그런데 SDK 버전이 올라가면서 `BaseApp.Commit` 시점에 **특정 interval마다 스냅샷을 생성하는 기능**([`go app.snapshotManager.SnapshotIfApplicable(header.Height)`](https://github.com/cosmos/cosmos-sdk/blob/0c4872928a65b7f301b39ee85e794d4bc869f635/baseapp/abci.go#L491))이 goroutine으로 비동기 실행되도록 변경되었고, 이로 인해 기존의 싱글 스레드 전제가 깨졌다.
...
- Cosmos SDK 체인의 `DelegatorDelegations` RPC를 호출하면서 `:authority` 헤더에 수동으로 값을 삽입해 동작 비교
