# Stateless In-Memory Cache — 비대칭 워크로드 환경에서의 분산 KV 스토리지 Scale-out
...
B2B SaaS 형태로 여러 노드의 RPC 엔드포인트를 제공하는 서비스를 운영하면서, 특정 노드에 예측 불가능한 트래픽 스파이크가 발생하는 문제를 격었습니다.

기존 구조로는 scale-out을 위해 노드를 추가하려면 최신 snapshot이 필요했기 때문에, hot standby 노드를 준비해도 갑작스러운 트래픽 spike에 즉각 대응하는 horizontal scale-out은 사실상 불가능했습니다. 최신 state snapshot은 100 GB 내외에 달하기도 하여, 유연한 scale-out을 위해 복수의 노드를 미리 준비해두는 것은 인프라 비용 측면에서도 큰 부담이었습니다.

핵심은 **RPC 서비스의 트래픽 특성**이었습니다. read 쓰루풀은 매우 높지만, write는 주기적 timestamp마다 한 번씩 batch로 발생하는 구조였습니다. write 워크로드와 read 워크로드를 완전히 분리하고, **stateless cache 레이어**를 별도로 구성하면 캐시 노드만 scale-out해서 RPC를 수행할 수 있다는 아이디어로부터 시작했습니다.
...
- **Write lock 동안 모든 read가 블로킹**: 주기적 write batch가 flush되는 순간 write lock을 잡으면, 그 시점에 대기 중인 read goroutine이 전부 멈추는 구간적 지연이 발생했습니다.
...
단조증가하는 timestamp는 MVCC(Multi-Version Concurrency Control) 방식에 자연스럽게 활용할 수 있습니다. write 시 lock을 잡는 대신 timestamp를 버전으로 삼아 버전을 나눠 읽고 쓸 수 있습니다. arena 기반 lock-free skiplist를 활용해 **iterator를 지원하는 in-memory KV cache**를 구현했습니다. arena 할당 방식은 개별 노드마다 별도의 메모리를 할당하지 않고 미리 확보한 연속 메모리 블록에서 순차 할당하므로, 최대한 GC 압박 없이 작동하도록 구현하였습니다.
...
중요 배포마다 k6로 stress 테스트(`vus=500`, `duration=10s`)를 실행해 이전 버전 대비 성능 회귀(regression)가 없는지 확인했습니다. 이 과정을 통해 각 최적화 단계의 실제 효과를 수치로 검증하고, 예상치 못한 성능 저하를 사전에 차단했습니다.

---

## 성과

| 항목 | 기존 (단일 노드) | Newmetric Edge 노드 (1 replica) |
|------|------|------|
...
단조증가하는 timestamp를 MVCC 버전으로 활용하면, write 시 lock 없이 다중 버전 읽기가 가능하고 arena를 안전하게 순환시킬 수 있습니다.
