...
핵심은 **RPC 서비스의 트래픽 특성**이었습니다. read 쓰루풋은 매우 높지만, write는 주기적 timestamp마다 한 번씩 batch로 발생하는 구조였습니다. write 워크로드와 read 워크로드를 완전히 분리하고, **stateless cache 레이어**를 별도로 구성하면 캐시 노드만 scale-out해서 RPC를 수행할 수 있다는 아이디어로부터 시작했습니다.
